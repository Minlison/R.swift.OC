...
struct CommanderOptions {
    ...
    static let platformDir = Option("platformDir", EnvironmentKeys.platformDir, description: "platformDir root folder that Xcode uses during build.")
}
...
//    let platformDir = try info.value(from: platformDir, name: "platformDir", key: EnvironmentKeys.platformDir)
    
    
    let outputURL = URL(fileURLWithPath: outputDir).appendingPathComponent(RSwiftOC.Generated.rawValue, isDirectory: false)
    ...
    let callInformation = CallInformation(
        ...
        platformDir: URL(fileURLWithPath: "/Applications/Xcode.app/Contents/Developer/Platforms/iPhoneOS.platform")
    )
    do {
