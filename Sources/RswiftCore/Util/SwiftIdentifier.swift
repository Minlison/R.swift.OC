...
struct SwiftIdentifier : CustomStringConvertible {
    ...
    private static func upercasePrefix(_ name: String) -> String {
        guard RswiftCore.isEdgeEnabled else {
            return name.lowercaseFirstCharacter
        }
        ...
        if prefixRange.location == NSNotFound {
            ...
            let upercasedPrefix = (name as NSString).substring(with: prefixRange).uppercased()
            return (name as NSString).replacingCharacters(in: prefixRange, with: upercasedPrefix)
        }
    }
...
private let SwiftKeywords = [
    ...
    "associatedtype", "class","Class", "deinit", "enum", "extension", "fileprivate", "func", "import", "init", "inout", "internal", "let", "open", "operator", "private", "protocol", "public", "static", "struct", "subscript", "typealias", "var",
